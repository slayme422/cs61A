...
def calc_eval(exp):
    ...
    if isinstance(exp, Pair):
        ...
        return exp
    #Modify calc_eval to allow the lookup of symbols in bindings and get their values.
...
def floor_div(args):
    ...
    while next_args!=nil:
        ...
        #除数为0的情况
        if isinstance(next_args.first, Pair):
            divisor = calc_eval(next_args.first)  
        else:
            divisor= next_args.first  

            if divisor==0:
                raise ZeroDivisionError
        dividend=dividend // divisor
        next_args=next_args.rest
...
def eval_and(expressions):
    ...
    while cur!=nil:
        ...
        if isinstance(cur.first,Pair):
            cur_val=calc_eval(cur.first)
        else:
            cur_val=cur.first

        cur=cur.rest

        if cur_val is scheme_f:
            return False
...
def eval_define(expressions):
    ...
    bindings[symbol]=calc_eval(expressions.rest.first)
    return symbol
