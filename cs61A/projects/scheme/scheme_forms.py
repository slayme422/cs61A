...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(signature):
        ...
        # BEGIN PROBLEM 10
        "*** YOUR CODE HERE ***"
        #拆分signature的如(define (f x y) (+ x y))改成(define f (lambda (x y) (+ x y)))
        symbol=signature.first#f
        formals=signature.rest#x y
        body=expressions.rest
        
        lambda_expr=Pair('lambda',Pair(formals,body))
        new_expr=Pair(symbol,Pair(lambda_expr,nil))
        return do_define_form(new_expr,env)
        # END PROBLEM 10
    else:
...
def make_let_frame(bindings, env):
    ...
    while bindings is not nil:
        ...
        if name in seen_names:
            raise SchemeError("Duplicate variable name in let bindings")
        if not scheme_symbolp(name): raise SchemeError(...)
        seen_names.append(name)
        
        val_expr = binding.rest.first
        val = scheme_eval(val_expr, env)
