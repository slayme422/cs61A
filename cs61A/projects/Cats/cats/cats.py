...
def pick(paragraphs, select, k):
    ...
    ''
    """
    selected_parag=[p for p in paragraphs if select(p)]
    if len(selected_parag) > k :
        return selected_parag[k]
    else:
        return ''
...
def about(subject):
    ...
    def select(paragraph):
        #遍历subject,列表推导式：从一个列表创建出新的列表
        lower_str=lower(paragraph)
        no_punctuation=remove_punctuation(lower_str)
...
def accuracy(typed, source):
    ...
    return (correctness/typed_length)* 100
...
def wpm(typed, elapsed):
    ...
    #按字符个数计算每分钟的时间 character-per-minute
    cpm=(characters/elapsed) * 60

    #用cpm转换成wpm(words-per-minute) 每5个字符串算做一个单词
    wpm= float (cpm / 5)
...
def memo(f):
    """A general mqemoization decorator."""
    cache = {}
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    # BEGIN PROBLEM 5
    "*** YOUR CODE HERE ***"
    #如果就是原本的输入单词，那就直接返回输入的单词
    if typed_word in word_list:
        return typed_word
    
    
    lowest_degree= limit+1
    closest_word = typed_word
    
    #区别度，根据不同的函数，区别度不同
    for word in word_list:
        differences_degree=diff_function(typed_word , word, limit)
        if differences_degree <= limit and differences_degree < lowest_degree:
            
            lowest_degree= differences_degree
            closest_word = word
    
    if lowest_degree>limit:
        return typed_word
...
def minimum_mewtations(typed, source, limit):
    ...
    if limit<0:
        return float('inf')
    if not typed :
        return len(source) # Base cases should go here, you may add more base cases as needed.
    if not source:
        return len(typed)
        # BEGIN
        "*** YOUR CODE HERE ***"
...
def report_progress(typed, source, user_id, upload):
    ...
    >>> print_progress = lambda d: print('ID:', d['id'], 'Progres', d['progress'])
...
def time_per_word(words, timestamps_per_player):
    ...
    for single_tpp in tpp:
        duration=[]
        for i in range(len(single_tpp)-1):
            duration.append(single_tpp[i+1]-single_tpp[i])
...
def fastest_words(words_and_times):
    ...
    for _ in player_indices:
        player_fast_list=[]
        all_fastest_word.append(player_fast_list)
    ...
    for word_index in word_indices:
        ...
        for player_index in player_indices:
            current_player_word_time=times[player_index][word_index]
            #如果魔偶人比这个快就把时间词语放进对应的词组all_fastest_word里
            if current_player_word_time< fastest_time:
                fastest_time=current_player_word_time
                fastest_player=player_index
               
        all_fastest_word[fastest_player].append(words[word_index])


    return all_fastest_word
