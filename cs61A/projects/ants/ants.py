...
class Ant(Insect):
    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            #存在一个容器蚂蚁或者普通蚂蚁或者两个都无法互相容纳彼此就无法
        ...
        while current_place is not None and  not current_place.is_hive :     
            ...
            if ant:
                ...
                if ant.is_container:
                    ...
                    #被包的小朋友也要双倍 并且把ant的保护者成为container
                    container=ant.ant_contained
                    if container and not container.doubled:
                        container.damage*=2
                        container.doubled=True
                else:
                    #普通蚂蚁
                    if not ant.doubled:
                        ant.damage*=2
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        """
        #LongThrower只能击中从第五个格以外的，近战打不了
        # BEGIN Problem 3 and 4
        nextbro=self.place
        current_range=0
        
        while not nextbro.is_hive:
            if nextbro.bees and current_range>=self.lower_bound and current_range<=self.upper_bound:
                return random_bee(nextbro.bees)          
            else:
                current_range+=1
                nextbro=nextbro.entrance
        return None
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ##有反弹伤害，收到多少伤害就给别的所有这个地方的蜜蜂造成多少伤害
        #死亡的时候给所有在这个地方的蜜蜂造成damage
        ...
        # Then reduce own health
        current_place = self.place
        
        super().reduce_health(amount)   
                
        # Deal death damage to all bees in current place
        #蚂蚁要是死掉会引发ant.place 成为 None
...
class ContainerAnt(Ant):
    ...
    def store_ant(self, ant):
        # BEGIN Problem 8a
        "*** YOUR CODE HERE ***"
        if not ant.is_container and not self.ant_contained:
...
class TankAnt(ContainerAnt):
    ...
    def action(self, gamestate):
       ...
       # 首先攻击所有蜜蜂
        ...
        # 然后调用原本的 action（例如 ThrowerAnt 可能会继续扔叶子）
